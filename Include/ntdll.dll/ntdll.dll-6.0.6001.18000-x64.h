...
typedef struct _RTL_STACK_TRACE_ENTRY {
    ...
    struct _RTL_STACK_TRACE_ENTRY *FreeChain;
...
typedef struct _STACK_TRACE_DATABASE {
    ...
    struct _RTL_STACK_TRACE_ENTRY *FreeLists[32];
...
TppCallbackpAvailableWorkerCount;
...
InternalVerifyAndCommitUILanguageSettings();
IsDebugPortPresent();
IsDot();
IsHangulL();
IsHangulLV();
