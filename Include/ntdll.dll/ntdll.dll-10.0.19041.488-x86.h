...
typedef struct _EPROCESS {
    ...
    PULONG AllowedCpuSetsIndirect;
    PULONG DefaultCpuSetsIndirect;
...
typedef struct _ETHREAD {
    ...
    PULONG SelectedCpuSetsIndirect;
    struct _EJOB *Silo;
...
typedef struct _HEAP_LARGE_ALLOC_DATA {
    ...
    ULONG UnusedBytes : 16;
    ULONG ExtraPresent : 1;
    ULONG GuardPageCount : 1;
    ULONG GuardPageAlignment : 6;
    ULONG Spare : 4;
    ULONG AllocatedPages : 20;
};
...
typedef struct _HEAP_LFH_BUCKET {
    ...
    ULONG TotalSubsegmentCount;
    ULONG ReciprocalBlockSize;
    UCHAR Shift;
    UCHAR ContentionCount;
    ULONG AffinityMappingLock;
...
typedef struct _HEAP_LFH_SUBSEGMENT_OWNER {
    ...
    UCHAR SlotIndex;
    UCHAR Spare1;
    ULONG AvailableSubsegmentCount;
    ULONG Lock;
...
typedef struct _HEAP_PAGE_RANGE_DESCRIPTOR {
    ...
    USHORT Spare0 : 15;
    UCHAR RangeFlags;
    UCHAR RangeFlagBits : 5;
    UCHAR CommittedPageCount : 3;
...
typedef struct _HEAP_VS_CONTEXT {
    ...
    ULONG TotalCommittedUnits;
    ULONG FreeCommittedUnits;
...
typedef struct _KNODE {
    ...
    ULONG IdleSmtSet;
    ULONG NonPairedSmtSet;
    ULONG IdleCpuSet;
    ULONG DeepIdleSet;
    ULONG IdleConstrainedSet;
    ULONG NonParkedSet;
    ULONG SoftParkedSet;
    ULONG NonIsrTargetedSet;
    LONG ParkLock;
    USHORT ThreadSeed;
    USHORT ProcessSeed;
    ULONG SiblingMask;
    ...
    ULONG PpmConfiguredQosSets[5];
    ULONG Spare11;
    union _KQOS_GROUPING_SETS QosGroupingSets;
    ULONG QosPreemptibleSet;
    ULONG LLCLeaders;
};
...
typedef struct _KPRCB {
    ...
    ULONG DbgLogs[512];
    ULONG DbgCount;
    ULONG PrcbPadRemainingPage[499];
...
typedef struct _KTIMER_TABLE {
    ...
    struct _KTIMER_TABLE_ENTRY TimerEntries[1][256];
...
typedef struct _KTIMER_TABLE_STATE {
    ULONG64 LastTimerExpiration[1];
    ULONG LastTimerHand[1];
};
...
typedef struct _MMSUPPORT_INSTANCE {
    ...
    ULONG MinimumWorkingSetSize;
    ULONG WorkingSetLeafSize;
    ULONG WorkingSetLeafPrivateSize;
...
typedef struct _RTLP_HP_ALLOC_TRACKER {
    ...
    UCHAR AllocTrackerBitmapBuffer[40];
};
...
typedef struct _RTL_CSPARSE_BITMAP {
    ...
    ULONG BitCount;
    ULONG BitmapLock;
    ULONG DecommitPageIndex;
    ULONG RtlpCSparseBitmapWakeLock;
    UCHAR LockType;
    UCHAR AddressSpace;
    UCHAR MemType;
    UCHAR AllocAlignment;
    ULONG CommitDirectoryMaxSize;
    ULONG CommitDirectory[1];
};
...
typedef struct _RTL_HEAP_MEMORY_LIMIT_DATA {
    ...
    ULONG CommitLimitFailureCode;
    ULONG MaxAllocationSizeBytes;
    ULONG AllocationLimitFailureCode;
};
...
LdrDelegatedKiRaiseUserExceptionDispatcher;
LdrDelegatedKiUserApcDispatcher;
LdrDelegatedKiUserCallbackDispatcher;
LdrDelegatedKiUserExceptionDispatcher;
LdrDelegatedLdrInitializeThunk;
LdrDelegatedRtlDispatchAPC;
LdrDelegatedRtlUserThreadStart;
LdrDelegatedSystemDllInitBlock;
LdrFatalInitError;
LdrInitState;
...
LdrpPageSize;
LdrpParentInterlockedPopEntrySListPointer;
LdrpParentRtlInitializeNtUserPfnPointer;
LdrpParentRtlResetNtUserPfnPointer;
LdrpParentRtlRetrieveNtUserPfnPointer;
LdrpPathLock;
LdrpPolicyBits;
...
_imp__NtClose;
_imp__NtCreateFile;
_imp__NtDeviceIoControlFile;
_imp__NtOpenKey;
_imp__NtQueryValueKey;
_imp__RtlInitUnicodeString;
_imp__ZwClose;
_imp__ZwOpenKey;
_imp__ZwQueryInformationToken;
...
LdrpDestroyNode();
LdrpDetectDetour();
LdrpDllCharacteristicsToLoadFlags();
LdrpDoDebuggerBreak();
LdrpDoPostSnapWork();
LdrpDrainWorkQueue();
...
LdrpValidateUserCallTargetBitMapCheck1();
LdrpValidateUserCallTargetBitMapRet();
...
PsspFilterReadFaultsFilter();
PsspFreeLinkedHandleList();
PsspHandleDumper();
PsspHandleStreamSizeCalculator();
PsspQueryInfoClass_10();
PsspQueryInfoClass_9();
PsspQueryInfoClass_PSS_QUERY_HANDLE_TRACE_INFORMATION();
PsspQueryInfoClass_PSS_QUERY_PROCESS_INFORMATION();
PsspQueryInfoClass_PSS_QUERY_THREAD_INFORMATION();
PsspQueryVmBulkMode();
PsspSampleCounters();
PsspWalkHandleTable();
...
QueryRegistryValue();
RTL_BINARY_ARRAY<RTLP_FLS_CALLBACK_ENTRY,8,4>::ChunkAllocate();
...
RtlInitializeProcessExceptionChain();
RtlInitializeRXact();
...
RtlpFcQueryAllFeatureUsageSubscriptionNotificationsFromBuffer();
...
RtlpGetUserCallTargetBitMapRet();
...
RtlpHpSegReportUncommittedRange();
...
RtlpIsUserCallTargetBitMapCheckFault();
RtlpIsUtf8Process();
