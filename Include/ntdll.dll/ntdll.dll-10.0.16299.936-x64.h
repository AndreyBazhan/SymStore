...
typedef struct _HEAP_SEG_CONTEXT {
    ...
    struct _RTL_SRWLOCK SegmentLock;
...
typedef struct _KNODE {
    ...
    ULONG64 PpmConfiguredQosSets[3];
};
...
typedef struct _PROC_PERF_DOMAIN {
    ...
    struct _PROC_PERF_QOS_CLASS_POLICY QosPolicies[3];
    ULONG QosDisableReasons[3];
    ULONG QosEquivalencyMasks[3];
    UCHAR QosSupported;
    ULONG SelectionGeneration;
    struct _PERF_CONTROL_STATE_SELECTION QosSelection[3];
...
typedef struct _RTLP_HEAP_FEATURES {
    UCHAR UseSegmentHeap : 1;
    UCHAR EnableTagging : 1;
    UCHAR DisableDbgBreakOnCorruption : 1;
    UCHAR TraceLoggingEnabled : 1;
};

typedef struct _RTLP_HP_STACKTRACKING_CONTEXT {
    struct _RTL_SRWLOCK Lock;
    ULONG Flags;
    struct _RTL_STACKDB_CONTEXT StackDbContext;
};
...
typedef struct _RTL_CSPARSE_BITMAP {
    ...
    PULONG64 UserBitmap;
    ULONG64 BitCount;
    struct _RTL_SRWLOCK BitmapLock;
