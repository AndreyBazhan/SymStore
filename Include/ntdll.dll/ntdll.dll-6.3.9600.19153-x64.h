...
typedef struct _EPROCESS {
    ...
    ULONG MemoryDisambiguationDisable : 1;
    ULONG Flags3Unused : 11;
    ULONG AddressPolicyFrozen : 1;
    LONG SvmReserved;
    PVOID SvmReserved1;
    ULONG64 SvmReserved2;
...
typedef struct _HEAP_LFH_CONTEXT {
    PVOID BackendCtx;
    struct _HEAP_LFH_CALLBACKS Callbacks;
    struct _HEAP_LFH_BUCKET Buckets[129];
    struct _HEAP_LFH_AFFINITY_SLOT AffinitySlots[129];
};
...
typedef struct _HEAP_LFH_SUBSEGMENT {
    ...
    UCHAR Location;
    UCHAR DecommitInProgress;
    ULONG64 AllocationBitmap[1];
};
...
typedef struct _SEGMENT_HEAP {
    ...
    struct _HEAP_LOCK SegmentAllocatorLock;
    struct _LIST_ENTRY SegmentListHead;
    ULONG64 SegmentCount;
    struct _RTL_RB_TREE FreePageRanges;
    PVOID NextLocalInfoAddress;
    PVOID NextUncommitedAddress;
    PVOID ReservedLimit;
...
AitProcessEndEvent;
AitSystemUsageByDllEvent;
AitSystemUsageByExeEvent;
AitpEtwRegHandle;
AitpFirstCall;
AltResMemBlockCount;
...
AeGetImageHeaderHashForCurrentProcess$filt$0();
...
LdrpFindOrMapDependency$fin$0();
LdrpFindOrMapDll();
LdrpFindOrMapDll$fin$0();
LdrpFindTlsEntry();
LdrpFreeSnapContext();
LdrpFreeTls();
LdrpGenRandom();
...
RtlQueryInformationActivationContext$fin$1();
